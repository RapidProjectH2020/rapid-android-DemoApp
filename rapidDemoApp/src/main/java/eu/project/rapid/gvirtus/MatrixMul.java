...
public class MatrixMul extends Remoteable {
    public static final String LOG_TAG="MATRIXMUL";
    ...
    public static void registerProviders() {
        ...
        //providers.setDefaultProvider(providers.getBest());
    }
    ...
    public MatrixMul(DFE dfe) {
        ...
        try {
            ...
            Log.v(LOG_TAG, "Read the PTX source");
            Log.v(LOG_TAG, ptxSource);
    ...
    public boolean localGpuMatrixMul(int widthA, int heightA, int widthB) {
        Log.v(LOG_TAG, "Entered matrixMul");
        ...
        if (bestProvider!=null) {
            Log.d(LOG_TAG,"Best provider:"+bestProvider.getHost()+":"+bestProvider.getPort());
            try {
                ...
                driver.close();
                Log.v(LOG_TAG, "matrixMul 14");
                return correct;

            } catch (IOException ex) {
                Log.e(LOG_TAG, "Error while running MatrixMul: " + ex);
                throw new RuntimeException(ex);
            }
        }
        return false;
